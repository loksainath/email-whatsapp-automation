...
# =================================================
# AUTH GUARD (NO SESSION API CALLS ❗)
# =================================================
def login_required(f):
    @wraps(f)
    def wrapper(*args, **kwargs):
        # Auth server handles session
        return f(*args, **kwargs)
    return wrapper
...
@app.route("/profile")
def profile():
    return redirect(f"{AUTH_SERVER}/profile")

@app.route("/logout")
def logout():
    return redirect(f"{AUTH_SERVER}/login")
